...
export function convertFieldsToArray(options: SearchParams): string {
	...
	if (options.targets) {
		newOptions.targets = Object.keys(options.targets).filter(
			(key) => options.targets?.[key as TargetsField],
		)
	}

	if (options.fields) {
		newOptions.fields = Object.keys(options.fields).filter(
			(key) => options.fields?.[key as keyof SearchField],
		)
	}
...
function convertFiltersToUrlString(filters: FilterField): string {
	...
	for (let [field, conditions] of Object.entries(filters)) {
